...
const Category = () => {
  const [myFilters, setMyFilters, loading] = useState({
    filters: { category: [] },
  });
  ...
  const loadMore = () => {
    ...
    getFilteredProducts(toSkip, limit, myFilters.filters, loading).then(
      (data) => {
        if (data.error) {
          setError(data.error);
        } else {
  ...
  const loadMoreButton = () => {
    return size > 0 && size >= limit && loadMore();
  };
